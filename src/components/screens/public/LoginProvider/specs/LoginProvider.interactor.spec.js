...
import Provider from '@/components/screens/public/LoginProvider/modules/Provider';
  ...
  describe('behavior', () => {
    ...
    describe('should takes query params from current location and check hydra oauth skip status', () => {
      ...
      it('should show error if check oauth if fault', async () => {
        ...
        expect(wrapper.find(Provider).attributes().error).toBe('');
    ...
    it('should cancel and close login', () => {
      ...
      wrapper.find(Provider).vm.$emit('close');
    ...
    it('should handle password submit and makes hydra login', async () => {
      ...
      wrapper.find(Provider).vm.$emit('complete', { redirect: 'new/path' });
